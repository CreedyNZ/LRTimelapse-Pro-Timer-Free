...
char localKey = 'y';						// The current pressed key
char lastKeyPressed = 'Z';				// The last pressed key

unsigned long lastKeyCheckTime = 0;
unsigned long lastKeyPressTime = 0;
unsigned long lastDispTurnedOffTime = 0;
const  int keySampleRate          = 0;      // ms between checking keypad for key
...
void setup() {
 ...
 // lcd.begin(16, 2);
  ...
  analogWrite(BACK_LIGHT, 200);
  delay(5000);
  //DimLCD(1024,act_BackLightBrightness(),2);
  ...
  if (millis() > lastKeyCheckTime + keySampleRate) {
    ...
    if (localKey != lastKeyPressed) {
     ...
     // keypad.RepeatRate = keyRepeatRateSlow;
      ...
      if (localKey != 0 && millis() > lastKeyPressTime) {
        // yes, repeat this key
        if ( (localKey == 2 ) || ( localKey == 8 ) || ( localKey == '#' ) ) {
          processKey();
        }
      }
    }
